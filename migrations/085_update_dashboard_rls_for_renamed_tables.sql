-- Update RLS policies for renamed dashboard tables
-- Date: 2025-11-09
-- Purpose: Update RLS policies to reference the new table names
...
ON users_dashboard_config FOR SELECT
USING (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_config.site_id
...
ON users_dashboard_config FOR UPDATE
USING (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_config.site_id
...
ON users_dashboard_pages FOR SELECT
USING (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_pages.site_id
        AND sites.user_id = auth.uid()
    )
);
...
ON users_dashboard_pages FOR INSERT
WITH CHECK (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_pages.site_id
...
ON users_dashboard_pages FOR UPDATE
USING (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_pages.site_id
...
ON users_dashboard_pages FOR DELETE
USING (
    EXISTS (
        SELECT 1 FROM sites
        WHERE sites.id = users_dashboard_pages.site_id
...
COMMENT ON POLICY "Super admins can manage all dashboard configs" ON users_dashboard_config IS
    'Platform super_admins have full access to all dashboard configurations for support and management.';

COMMENT ON POLICY "Site owners can view their dashboard config" ON users_dashboard_config IS
    'Site owners (user_id in sites table) can view their own site dashboard configuration.';

COMMENT ON POLICY "End users can view dashboard configs" ON users_dashboard_config IS
    'All authenticated users can view dashboard configs. Future: Add membership/subscription checks.';

COMMENT ON POLICY "Super admins can manage all dashboard pages" ON users_dashboard_pages IS
    'Platform super_admins have full access to all dashboard pages for support and management.';

COMMENT ON POLICY "Site owners can view their dashboard pages" ON users_dashboard_pages IS
    'Site owners can view all dashboard pages for their sites (both published and unpublished).';

COMMENT ON POLICY "End users can view published dashboard pages" ON users_dashboard_pages IS
