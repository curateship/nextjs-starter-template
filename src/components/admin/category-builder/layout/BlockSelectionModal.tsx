...
export function BlockSelectionModal({
  ...
  const disabledBlocks = useMemo(() => {
    ...
    CATEGORY_BLOCK_TYPES.forEach(blockType => {
  ...
  const getConflictMessage = (blockType: typeof CATEGORY_BLOCK_TYPES[0]): string | null => {
    if (!blockType.conflictsWith) return null

    for (const conflictType of blockType.conflictsWith) {
      if (existingBlockTypes.includes(conflictType)) {
        const conflictBlock = CATEGORY_BLOCK_TYPES.find(b => b.type === conflictType)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[900px] max-w-[95vw] sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the blocks you want to add to your category. You can add multiple blocks at once and specify quantities.
          </DialogDescription>
        </DialogHeader>
            ...
            const blockCard = (
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className={`flex items-center justify-center w-10 h-10 rounded ${isSelected ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {isDisabled && <Info className="w-4 h-4 text-muted-foreground flex-shrink-0" />}
                    </div>
                    ...
                    {isSelected && (
                      <div className="flex items-center space-x-2 mt-3" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center space-x-1">
                          <Button variant="outline" size="sm" className="h-6 w-6 p-0" onClick={(e) => { e.stopPropagation(); handleQuantityChange(blockType.type, -1) }}>
                            <Minus className="w-3 h-3" />
                          </Button>
                          <span className="text-sm font-medium w-6 text-center">{quantity}</span>
                          <Button variant="outline" size="sm" className="h-6 w-6 p-0" onClick={(e) => { e.stopPropagation(); handleQuantityChange(blockType.type, 1) }} disabled={quantity >= 10}>
            ...
            if (isDisabled && conflictMessage) {
              return (
                <TooltipProvider key={blockType.type}>
                  <Tooltip>
                    <TooltipTrigger asChild>{blockCard}</TooltipTrigger>
                    <TooltipContent><p>{conflictMessage}</p></TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )
            }
