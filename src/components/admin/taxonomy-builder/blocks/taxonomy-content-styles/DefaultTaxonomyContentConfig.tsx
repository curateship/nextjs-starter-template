...
import type { TaxonomyContentStyleAdminProps } from "./index"

export function DefaultTaxonomyContentConfig({ config, onConfigChange }: TaxonomyContentStyleAdminProps) {
          ...
          <div className="flex gap-4">
            {(['left', 'center'] as const).map((option) => (
              <div key={option} className="flex items-center gap-2">
                <Checkbox
                  id={`taxonomy-alignment-${option}`}
                  checked={alignment === option}
                  onCheckedChange={() => onConfigChange('alignment', option)}
                />
                <Label htmlFor={`taxonomy-alignment-${option}`} className="text-sm capitalize cursor-pointer">{option}</Label>
              </div>
            ))}
          </div>
        </CardContent>
          ...
          <div className="flex gap-4">
            {([
              ...
              <div key={option.key} className="flex items-center gap-2">
                <Checkbox
                  id={`taxonomy-title-size-${option.key}`}
                  checked={titleSize === option.key}
                  onCheckedChange={() => onConfigChange('titleSize', option.key)}
                />
                <Label htmlFor={`taxonomy-title-size-${option.key}`} className="text-sm cursor-pointer">{option.label}</Label>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <Label htmlFor="taxonomyContentMaxWidth" className="text-sm">Max width</Label>
            <input
              id="taxonomyContentMaxWidth"
              type="number"
              min="480"
              max="1280"
