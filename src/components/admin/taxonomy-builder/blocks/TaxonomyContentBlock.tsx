...
import { TAXONOMY_CONTENT_STYLES } from "./taxonomy-content-styles"
import {
  Bold,
  Italic,
  List,
...
interface TaxonomyContentBlockProps {
  ...
  onTaxonomyTitleChange?: (title: string) => void
}

export function TaxonomyContentBlock({ content, onContentChange, siteId, blockId, taxonomyData, onTaxonomyTitleChange }: TaxonomyContentBlockProps) {
  const [activeTab, setActiveTab] = useState('content')
  const [isImagePickerOpen, setIsImagePickerOpen] = useState(false)
  const [localTitle, setLocalTitle] = useState(taxonomyData?.title || taxonomyData?.name || 'Untitled Term')
  ...
  // Update local title when taxonomy data changes
  useEffect(() => {
    setLocalTitle(taxonomyData?.title || taxonomyData?.name || 'Untitled Term')
  }, [taxonomyData?.title, taxonomyData?.name])
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: 'Write your taxonomy description here...',
      }),
      Image.configure({
        HTMLAttributes: {
  ...
  const handleTitleChange = (value: string) => {
    setLocalTitle(value)
    if (onTaxonomyTitleChange) {
      onTaxonomyTitleChange(value)
    }
  }
  ...
  const ActivePanel = TAXONOMY_CONTENT_STYLES[taxonomyContentStyle]?.AdminPanel
      ...
      <TabsContent value="content" className="mt-6">
        ...
        <div className="space-y-2 mb-4 px-6">
          ...
          <div className="grid grid-cols-2 gap-2 max-w-sm">
            {Object.entries(TAXONOMY_CONTENT_STYLES).map(([key, style]) => (
              <button
                key={key}
                type="button"
        ...
        <div className="space-y-2 px-6 mb-4">
          <Label htmlFor="taxonomy-title">Term Title</Label>
          <Input
            id="taxonomy-title"
            value={localTitle}
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">Display the taxonomy featured image</p>
              </div>
              <Switch
