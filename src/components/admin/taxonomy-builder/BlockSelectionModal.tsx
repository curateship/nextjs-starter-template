...
export function BlockSelectionModal({
  ...
  const disabledBlocks = useMemo(() => {
    ...
    TAXONOMY_BLOCK_TYPES.forEach(blockType => {
  ...
  const getConflictMessage = (blockType: typeof TAXONOMY_BLOCK_TYPES[0]): string | null => {
    if (!blockType.conflictsWith) return null

    for (const conflictType of blockType.conflictsWith) {
      if (existingBlockTypes.includes(conflictType)) {
        const conflictBlock = TAXONOMY_BLOCK_TYPES.find(b => b.type === conflictType)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[900px] max-w-[95vw] sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the blocks you want to add to your taxonomy. You can add multiple blocks at once and specify quantities.
          </DialogDescription>
        </DialogHeader>

        <div className="grid grid-cols-3 gap-4 py-4">
          {TAXONOMY_BLOCK_TYPES.filter(bt => bt.type !== 'taxonomy-content').map((blockType) => {
