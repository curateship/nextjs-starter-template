...
export function TaxonomyPreview({ blocks, taxonomy, site, className = "", blocksLoading = false }: TaxonomyPreviewProps) {
  // Convert taxonomy blocks to PreviewBlock format for the generic preview system
  ...
  const previewTaxonomy = {
    id: taxonomy?.id || 'preview',
    title: taxonomy?.title || 'Preview Taxonomy',
    slug: taxonomy?.slug || 'preview',
    description: taxonomy?.description || null,
    featured_image: taxonomy?.featured_image || null,
          ...
          {blocksLoading ? (
            ...
            <div className="flex items-center justify-center min-h-[400px] text-muted-foreground">
              <div className="text-center">
                ...
                <div className="text-sm">Add blocks to see your taxonomy preview</div>
              </div>
            </div>
          ) : (
            <TaxonomyBlockRenderer site={previewSite} taxonomy={previewTaxonomy} />
          )}
        </div>
      </div>
    </div>
