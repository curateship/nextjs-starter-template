...
interface UpdateUserUserPageData {
  title?: string
  slug?: string
...
interface UserPageSettingsModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  page: UserPage | null
  site: SiteWithTheme | null
  onSuccess?: (updatedPage: UserPage) => void
}

export function UserPageSettingsModal({
  open,
  onOpenChange,
  page,
  site,
  onSuccess
}: UserPageSettingsModalProps) {
  const [formData, setFormData] = useState<UpdateUserUserPageData>({})
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (!response.ok || result.error) {
        setError(result.error || 'Failed to save user page as draft')
        return
      }

      if (result.data) {
        setSaveMessage('User page saved as draft')
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!response.ok || result.error) {
        setError(result.error || 'Failed to publish user page')
        return
      }

      if (result.data) {
        setSaveMessage(page?.is_published ? 'User page updated' : 'User page published')
        setLastSavedAt(new Date())
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card className="shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Configure the basic settings for this user page
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <div>
                    <Label htmlFor="modal-is_default">Set as default page</Label>
                    <p className="text-xs text-muted-foreground">
                      This page will be the first page users see when accessing their dashboard
                    </p>
                  </div>
                </div>
