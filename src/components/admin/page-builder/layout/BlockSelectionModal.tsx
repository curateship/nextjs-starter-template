...
export function BlockSelectionModal({
  ...
  const disabledBlocks = useMemo(() => {
    ...
    PAGE_BLOCK_TYPES.forEach(blockType => {
  ...
  const getConflictMessage = (blockType: typeof PAGE_BLOCK_TYPES[0]): string | null => {
    if (!blockType.conflictsWith) return null

    for (const conflictType of blockType.conflictsWith) {
      if (existingBlockTypes.includes(conflictType)) {
        const conflictBlock = PAGE_BLOCK_TYPES.find(b => b.type === conflictType)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[900px] max-w-[95vw] sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the blocks you want to add to your page. You can add multiple blocks at once and specify quantities.
          </DialogDescription>
        </DialogHeader>
