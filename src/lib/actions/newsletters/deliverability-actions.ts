...
export async function getDeliverabilityReport(siteId: string): Promise<{ data: DeliverabilityReport | null; error: string | null }> {
  try {
    ...
    const { data: contacts } = await supabaseAdmin
      ...
      .select('status, engagement_score, last_engaged_at')
      .eq('site_id', siteId)
    ...
    for (const c of contacts ?? []) {
      if (c.status === 'active') {
        ...
        if (!c.last_engaged_at || c.last_engaged_at < ninetyDaysAgo) {
          contactHealth.cold++
        }
      }
    ...
    for (const e of events ?? []) {
      ...
      else if (e.event_type === 'opened') totalOpened++
      else if (e.event_type === 'clicked') totalClicked++
      else if (e.event_type === 'bounced') totalBounced++
      else if (e.event_type === 'complained') totalComplained++
    }
