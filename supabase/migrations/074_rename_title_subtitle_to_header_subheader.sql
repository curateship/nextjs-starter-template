...
-- Blocks affected: faq, product-hotspot, listing-views, product-video
...
SET content_blocks = (
  SELECT jsonb_object_agg(
    ...
    CASE
      ...
      WHEN block_key IN ('faq', 'product-hotspot', 'listing-views', 'product-video') THEN (
        SELECT jsonb_object_agg(
          CASE
...
WHERE content_blocks IS NOT NULL
  AND EXISTS (
    SELECT 1
    FROM jsonb_each(content_blocks) AS blocks(block_key, block_value)
    WHERE block_key IN ('faq', 'product-hotspot', 'listing-views', 'product-video')
      AND (
