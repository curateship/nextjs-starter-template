...
- **Components are filed by feature, not left loose in `src/components/`.** Custom Shell's layout is the reference: `account/`, `admin/`, `feedback/`, `media/`, `settings/`, `shell/` (app frame and page frames), `shared/` (cross-feature pieces that are not primitives — `dashboard-table`, `dashboard-toolbar`, `image-upload`, `pricing-table`), plus `ui/` for the primitives. A new screen goes in the folder for the part of the app it belongs to; only put something in `shared/` once a second feature actually uses it.
...
The Custom Shell app has a per-workspace **Styling** settings tab (`/admin/settings/styling`, `src/components/settings/styling-settings.tsx`) that retunes spacing, borders, backgrounds, and modal chrome at runtime. Values live on `ShellConfig.styling` (`src/lib/custom-shell.tsx`), persist per-workspace (jsonb, no migration), and apply through CSS variables. The defaults below reproduce the standard look — only the mechanism changed. **New UI in this app must consume these variables (with the documented fallback) instead of hardcoding spacing/border/background, or the user's styling silently won't reach it.**

`createDefaultStyling()` in `src/lib/custom-shell.tsx` owns the built-in fallback. Apps created by Personal IDE replace `src/lib/scaffold-styling.ts` with the complete styling currently saved in Custom Shell, so their first workspace and Styling reset reproduce that captured look. Change the fallback in `createDefaultStyling()` only when Custom Shell itself needs a new unsaved baseline; do not hardcode styling per screen.
...
- **Sidebar + sticky header.** Both render with `bg-sidebar` (`var(--sidebar)`); recolor them together by overriding `--sidebar` on the shell wrapper (`src/components/shell/shell-layout.tsx`). Never give them separate backgrounds.
...
- Use the established Lucide action icons consistently: **`SettingsIcon` (the cog) for edit — always, everywhere**, `Trash2Icon` for delete, `PlusIcon` for add, and `Loader2Icon` for loading. **Never `PencilIcon`.** One icon means "change this thing", whether it opens a settings modal, an edit form or an inline field, so a cog is always the same promise and nobody has to learn which shape means which kind of editing.
...
- Spacing ladder: `gap-2` (8px) label to control, `gap-4` (16px) between fields and between a card's header and its content, `gap-6` (24px) between the stacked cards in a modal body (the `DialogBody` default). A **read-only card** — a spec sheet of one-line label-and-value rows, not fields — sits at `gap-2` instead: those rows are a list, and the field spacing makes them drift apart. A control dropped among them (a button, a link) takes the field spacing back with `mt-2`.
- **A footer whose buttons change with the tab needs a `key` per variant.** Both branches render a `DialogFooter`, so React keeps the button element and rewrites it — a primary "Done" becomes an outline "Cancel" in place, and `Button`'s own 150ms transition then animates the background from dark to light, so the button visibly flashes on every switch. Keying each footer swaps the elements instead of repainting one.
...
- Use the shared `DashboardTable` wrapper (`src/components/shared/dashboard-table.tsx` in Custom Shell), the `Table` primitives, and `TableSurface`. Do not build tables from raw div grids or one-off styling. Reference table: `src/components/admin/admin-users-dashboard.tsx`.
...
- **A multi-selection action is one request, and it says what it actually did.** Send the whole selection to a single bulk server function (`inArray`), never a `for` loop of single-item calls — a loop mid-failure leaves the job half done with no accounting, and ten rows means ten round trips to a remote database. The server enforces the same guards as the single-item version (the last workspace survives, the default plan cannot be archived) and returns the ids that went through and the ids that did not. The toast then counts honestly through the shared `describeBulkResult` (`src/lib/bulk-result.ts` in Custom Shell) — "3 workspaces deleted." or "2 workspaces deleted, 1 could not be deleted." — and the rows that did not go stay ticked. When nothing went through, that is a failure: `showErrorToast`, and leave the confirmation open.
...
- **Where the title has a target, the whole row is a click target for it.** Pass that same target to the shared `TableRow` as `rowAction` (`src/components/ui/table.tsx` in Custom Shell) — never a second, different action, and never a hand-rolled `onClick` on the `<tr>`. The row then gets `cursor-pointer` and the grey `hover:bg-muted/50` tint. A row with nothing to open gets no `rowAction` and stays flat, so the tint always means a click will do something. `rowAction` ignores clicks that land on a control, on the select column, on the actions column, and on a text selection being dragged; it also ignores modifier-clicks, which only a real link can honour. The row is deliberately not focusable — the title link or button is already the keyboard's way in, and a second tab stop per row would be worse for it.
- **The actions column is `TableCell column="actions"`, not `column="meta"`.** It looks identical; the difference is that a clickable row keeps its hands off it. This matters because a *disabled* button carries `pointer-events-none`, so a click aimed at it misses and lands on the cell behind — without the marker, clicking a greyed-out Delete would open the very thing it refused to delete.
...
- Footer is `DialogFooter` sitting directly on the modal surface — no border and no background band. **On the admin variant the body-to-footer gap belongs to the footer, not the body**: the body is a scroll viewport, so padding at its bottom is inside the scrolling box and slides away with the content, leaving the buttons flush on a card cut off mid-row. `theme.css` leaves only 8px in the body (just enough that a focus ring on the last control is not clipped, mirroring the top edge) and gives the footer the rest, so the visible gap is always the configured `--shell-modal-padding` whether or not the window scrolls. A window with no footer keeps its full bottom padding, and non-admin dialogs are unchanged. Never restate this per modal. It right-aligns buttons with `gap-2` and forces them to 36px (`h-9`); do not fight that height. Order is Cancel (`outline`), then the primary action; while submitting, disable both and show a `Loader2Icon` spinner inside the primary button.
- **Every footer reads the same way, left to right: `[Delete] ……… [Cancel] [Primary]`.** The button that throws something away (Delete, Remove) sits hard left with `className="mr-auto"` so it can never be mistaken for the primary; omit it when there is nothing to throw away. Cancel is always directly left of the primary and carries the same `disabled` as it, so a half-finished save cannot be walked away from. Primary labels follow one scheme: **"Save changes"** when editing something that already exists, **"Create <thing>"** when making a new one, and the action's own verb otherwise ("Select"). A window with nothing to save — a preview, a details view, an editor that auto-saves — ends with a single primary **"Done"** and no Cancel, because there is nothing a Cancel could undo. The convention is restated next to `DialogFooter` in `dialog.tsx`; keep the two in step. Modal titles are sentence case ("Edit workspace", never "Edit Workspace"), and a confirmation's title asks a question ("Delete this workspace?") with a `confirmLabel` that names what goes ("Delete workspace", not a bare "Delete").
- Failures never render inside the modal body, under a field, or inside a data surface: report them with the shared error toast (`showErrorToast` in `src/lib/error-toast.ts`, Custom Shell) — a red toast in the same fixed position as the success toasts that stays until dismissed, where a repeat failure replaces the previous one and starting a new attempt dismisses it. Keep entered values. Never call `toast.error` directly and never hand-roll an error paragraph, inline red text, or boxed alert. **Field validation reports through the same toast**: fire it when the field is left (`onBlur`) or on submit, never on every keystroke, and keep `aria-invalid` on the input so the red ring still marks which field is at fault once the toast is dismissed. Retryable failures may add a `Try again` action to the toast.
...
- **A record is a window on its list, not a page of its own.** Where one item has more to say than a row, it opens as a modal from the list rather than as a route — Custom Shell's `admin-account-dialog.tsx` (from the Users table) and `account-dialog.tsx` (the member's own account) are the two references. Both put the tab group beside the title in `DialogHeader` (`flex flex-wrap items-center gap-x-4 gap-y-2`, `DialogTitle` then `TabsList`). Three rules make it hold together: the window takes only an **id** and loads its own data, so a link works for a record that is not on the page of the list you are looking at; **which record is open lives in the address** as `?open=<id>` (`readOpenSearch` in `src/lib/use-open-from-link.ts`) and navigates *without* `replace`, so Back closes the window while the list's own search and filters keep replacing; and **which tab is showing does not** — that is whether you clicked the row or its settings icon, not something to share. Give `DialogContent` one fixed height (`h-[48rem]`) sized to the tallest tab, or the window re-centres itself every time you switch. The tab holding typed work is `forceMount`ed with `hidden={tab !== …}` so a trip to another tab and back leaves the edits where they were, and the footer follows the tab: `[Cancel] [Save changes]` on the one that saves, a single `[Done]` on the ones that do not.
- **A modal that holds typed work uses the shared `FormDialog`** (`src/components/ui/form-dialog.tsx` in Custom Shell) rather than `Dialog` directly. Pass it `dirty` (the form's draft differs from what the window opened with) and `busy` (a save is running): a dirty window asks "Discard changes?" through `ConfirmDialog` before the backdrop, Escape, the X or Cancel can throw the edits away, a clean window still closes instantly, and a saving window refuses to close at all. Its `children` is a function handed the single `requestClose` — wire the footer's Cancel to it, or that path skips the question. Read-only, picker, and confirmation modals stay on plain `Dialog`.
...
- Reference form modal: `src/components/admin/admin-plans-dashboard.tsx` in Custom Shell (card sections, `FieldLabel` hints, full-width and paired fields, checkbox rows, footer). Its edit-account sibling `admin-users-dashboard.tsx` is the reference for a modal that only sends the fields that changed.
