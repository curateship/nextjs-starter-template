---
name: audit-change
description: Audit staged, unstaged, or recent code changes and automatically fix clear in-scope issues when the user authorized changes or a commit. Review-only requests remain read-only. Use when the user asks to audit, clean up, harden, simplify, remove bloat, or prepare changes for commit or merge, and after security-sensitive changes to authentication, server actions, API routes, middleware, database policies, uploads, payments, webhooks, or external integrations. Reading or running this skill never authorizes staging or committing; only an explicit user commit request does. Never push, deploy, or rewrite history.
---
...
- Keep requests that explicitly say review-only read-only. A request to change, fix, clean up, prepare for commit, or commit authorizes clear in-scope fixes found by the audit.
- Automatically apply every clear, in-scope fix without asking for confirmation, including when `commit-change` invoked the audit. Verify each logical fix before continuing.
- Do not ask permission for routine cleanup, simplification, Hard Cut deletion, regression fixes, security hardening, or a correction needed to make the requested change commit-ready.
- Stop for direction only when the fix needs authority the user has not granted, valid fixes conflict, or every viable resolution would risk data loss, weaken security, or materially change intended behavior. Explain the concrete conflict when that happens.
...
8. Automatically fix every clear in-scope finding when the request authorizes changes or a commit, then re-run the affected gates against the resulting diff.
...
- Stop after the verdict when the user requested review-only or did not explicitly request a commit.
- Do not leave a fixable Critical or Required finding for the user. Apply the smallest in-scope fix and re-audit. Stop for direction only under the authority and conflict limits above.
