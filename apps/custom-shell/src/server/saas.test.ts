...
describe("chargebacks", () => {
  const OPENED = Math.floor(new Date("2026-08-01").getTime() / 1_000)
  const DUE_BY = Math.floor(new Date("2026-08-20").getTime() / 1_000)

  /** A `charge.dispute.*` event, shaped like Stripe's real payloads. */
  function disputeEvent({
    id = "evt_dispute_1",
    type = "charge.dispute.created",
    status = "needs_response",
    amount = 4900,
    dueBy = DUE_BY as number | null,
    disputeId = "dp_1",
    livemode = false,
    created = OPENED,
  } = {}) {
    return {
      ...
      data: {
        object: {
          ...
          status,
          created,
          evidence_details: dueBy ? { due_by: dueBy } : {},
        },
      },
    } as never
  }

  /** Stripe's answer for the disputed charge, which is how the member is found. */
  const chargeFor = (customer: string | null) => async () =>
    ({ id: "ch_1", customer }) as never
  ...
  async function subscribedUser() {
    const user = await createUser()
    await seedStripeSubscription(user.id, { stripeCustomerId: "cus_disputed" })
    return user
  }

  function readDisputes() {
    return database.select().from(customShellDisputes)
  }

  it("records a dispute and matches it to the member who paid", async () => {
    ...
    expect(
      await applyStripeEvent(
        ...
        database,
        neverAsksStripe,
        chargeFor("cus_disputed")
      )
    ).toBe(true)

    const [row] = await readDisputes()
    expect(row.userId).toBe(user.id)
    expect(row.amountCents).toBe(4900)
    expect(row.status).toBe("needs_response")
    expect(row.reason).toBe("fraudulent")
    expect(row.evidenceDueBy).toEqual(new Date(DUE_BY * 1_000))
    expect(row.closedAt).toBeNull()
    // Test-mode disputes must link into the test dashboard, not the live one.
    expect(row.livemode).toBe(false)
  })

  it("records a dispute nobody can be matched to rather than dropping it", async () => {
    // Stripe cannot say whose charge it was, which must not lose the deadline.
    await applyStripeEvent(disputeEvent(), database, neverAsksStripe, () => {
  ...
  it("ignores a replayed delivery", async () => {
    await subscribedUser()
    const event = disputeEvent()

    expect(
      await applyStripeEvent(event, database, neverAsksStripe, chargeFor("cus_disputed"))
    ).toBe(true)
    expect(
      await applyStripeEvent(event, database, neverAsksStripe, chargeFor("cus_disputed"))
    ).toBe(false)

    expect(await readDisputes()).toHaveLength(1)
  })

  it("settles the same row when the dispute closes, and keeps the outcome", async () => {
    await subscribedUser()
    ...
    await applyStripeEvent(
      disputeEvent({ id: "evt_dispute_2", type: "charge.dispute.closed", status: "won" }),
      database,
      neverAsksStripe,
      chargeFor("cus_disputed")
    )

    const rows = await readDisputes()
    expect(rows).toHaveLength(1)
    expect(rows[0].status).toBe("won")
    expect(rows[0].closedAt).not.toBeNull()

    const { open, recent } = await listDisputes(database)
    expect(open).toHaveLength(0)
    expect(recent[0].statusLabel).toBe("Won")
  })

  it("does not restamp a dispute that already closed", async () => {
    await subscribedUser()
    for (const [index, status] of ["needs_response", "lost", "lost"].entries()) {
      await applyStripeEvent(
        disputeEvent({ id: `evt_close_${index}`, status }),
        database,
        neverAsksStripe,
        chargeFor("cus_disputed")
      )
    ...
    const [afterFirstClose] = await readDisputes()
    const closedAt = afterFirstClose.closedAt

    // A fourth event on the settled dispute — funds moving — must leave the
    // moment it was lost alone.
    await applyStripeEvent(
      disputeEvent({
        id: "evt_funds",
  ...
  it("counts an unfamiliar status as still open, so the alert never hides one", () => {
    expect(disputeIsOpen("needs_response")).toBe(true)
    expect(disputeIsOpen("under_review")).toBe(true)
    expect(disputeIsOpen("some_status_stripe_added_later")).toBe(true)
    expect(disputeIsOpen("won")).toBe(false)
    expect(disputeIsOpen("lost")).toBe(false)
    expect(disputeIsOpen("warning_closed")).toBe(false)
  })

  it("still alerts on an old dispute buried under newer settled ones", async () => {
    await subscribedUser()

    // One old dispute still running, then enough newer settled ones to push it
    // off the recent list. The alert has to find it anyway.
    await applyStripeEvent(
      disputeEvent({ id: "evt_old", disputeId: "dp_old", created: OPENED }),
      database,
      neverAsksStripe,
      chargeFor("cus_disputed")
    )
    for (let index = 0; index < 30; index += 1) {
      await applyStripeEvent(
        disputeEvent({
          id: `evt_new_${index}`,
          disputeId: `dp_new_${index}`,
          status: "won",
          // Each one strictly newer than the last, so the old dispute really is
          // pushed off the end of the recent list rather than maybe-ordered.
          created: OPENED + (index + 1) * 86_400,
        }),
        database,
        neverAsksStripe,
        chargeFor("cus_disputed")
    ...
    const { open, recent, total } = await listDisputes(database)
    expect(total).toBe(31)
    // The history list is capped, so the old one is not on it...
    expect(recent).toHaveLength(25)
    // ...but the alert still has it, which is the whole point.
    expect(open).toHaveLength(1)
    expect(open[0].stripeUrl).toContain("dp_old")
  })

  it("puts the soonest deadline first and counts every dispute", async () => {
    await subscribedUser()
    const later = Math.floor(new Date("2026-09-30").getTime() / 1_000)

    await applyStripeEvent(
      disputeEvent({ id: "evt_a", disputeId: "dp_late", dueBy: later }),
      database,
      neverAsksStripe,
      chargeFor("cus_disputed")
    )
    await applyStripeEvent(
      disputeEvent({ id: "evt_b", disputeId: "dp_soon" }),
      database,
      neverAsksStripe,
      chargeFor("cus_disputed")
    )
    // No deadline yet: an early warning sorts behind anything with a date.
    await applyStripeEvent(
      disputeEvent({
        id: "evt_c",
        disputeId: "dp_warning",
    ...
    const { open, total } = await listDisputes(database)
    expect(total).toBe(3)
    expect(open.map((row) => row.evidenceDueBy === null)).toEqual([
      false,
      false,
      true,
    ])
    expect(open[0].evidenceDueBy).toBe(new Date(DUE_BY * 1_000).toISOString())
    expect(open[0].stripeUrl).toBe(
      "https://dashboard.stripe.com/test/disputes/dp_soon"
    )
  })
})
