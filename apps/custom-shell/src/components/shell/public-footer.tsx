...
const SOCIAL_PATHS: Record<string, string> = {
  ...
  medium:
    "M13.54 12a6.77 6.77 0 1 1-13.54 0a6.77 6.77 0 0 1 13.54 0m7.42 0c0 3.55-1.51 6.42-3.38 6.42S14.2 15.55 14.2 12s1.51-6.42 3.38-6.42s3.38 2.87 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75S21.62 15.17 21.62 12s.53-5.75 1.19-5.75S24 8.83 24 12",
  substack:
...
function FooterLink({ link }: { link: PublicNavigationLink }) {
  const className = cn(
    "block rounded-md text-muted-foreground duration-150 hover:text-foreground",
    focusRing
  )

  if (isInternalHref(link.href)) {
    return (
      <Link {...toLinkProps(link.href)} className={className}>
        {link.label}
      </Link>
    )
  }
