...
export function renderBroadcastBlockHtml(block: BroadcastBlock): string {
  switch (block.kind) {
    case "header": {
      ...
      const inner = logoUrl
        ? `<img src="${escapeHtml(logoUrl)}" alt="Logo" width="${logoWidth}" style="width:${logoWidth}px;height:${logoHeight ? `${logoHeight}px` : "auto"};display:block;margin:0 ${alignment === "center" ? "auto" : "0"};border:0;outline:none;" />`
        : ""
