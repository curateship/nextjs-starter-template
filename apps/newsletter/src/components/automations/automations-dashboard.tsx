...
type SortColumn = "name" | "status" | "summary" | "updated"

const statusOrder: AutomationStatus[] = ["active", "paused", "draft"]
...
export function AutomationsDashboard({
    ...
    return automations
      .filter(
        (automation) =>
          ...
          automation.status.toLowerCase().includes(query) ||
      ...
      .sort((a, b) => {
        ...
        return (
          (Date.parse(a.updated_at) - Date.parse(b.updated_at)) * direction
        )
      })
  ...
  const confirmMassDelete = async () => {
    ...
    if (failedIds.length > 0) {
      setError(
        `${failedIds.length} Automation${failedIds.length === 1 ? "" : "s"} could not be deleted.`
      )
      return
    }
      ...
      <DashboardTable
        ...
        emptyText={
          searchQuery
            ...
            : "No automations yet. Create one to welcome, tag, and follow up with new contacts."
        }
        emptyColSpan={6}
      >
        {paginatedRows.map((automation) => {
          const badge = STATUS_BADGES[automation.status]
          return (
      ...
      >
        <DialogContent variant="admin" className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Deleting also removes their in-flight runs.
            </DialogDescription>
          </DialogHeader>
          <DialogBody>
            <p className="text-sm">
