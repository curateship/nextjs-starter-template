...
export const aiVideoLoginAttempts = pgTable(
  "login_attempts",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
  ...
  (table) => [
    index("ix_login_attempts_key_time").on(
      table.keyHash,
      table.attemptedAt
    ),
    index("ix_login_attempts_attempted_at").on(table.attemptedAt),
  ]
)
...
export const aiVideoMedia = pgTable(
  "media",
  {
    ...
    workspaceId: varchar("workspace_id", { length: 36 })
      .notNull()
      .references(() => aiVideoWorkspaces.id, { onDelete: "cascade" }),
  ...
  (table) => [
    ...
    index("ix_media_workspace_id").on(table.workspaceId),
    ...
    index("ix_media_workspace_type_created").on(
      table.workspaceId,
      table.fileType,
      table.createdAt
    ),
...
export const aiVideoGenerations = pgTable(
  ...
  (table) => [
    check(
      ...
      sql`${table.status} in ('draft', 'queued', 'writing_prompt', 'generating', 'saving', 'succeeded', 'failed')`
    ),
    index("ix_generations_user_id").on(table.userId),
    index("ix_generations_workspace_id").on(table.workspaceId),
    index("ix_generations_status").on(table.status),
    index("ix_generations_workspace_created").on(
      table.workspaceId,
      table.createdAt
    ),
  ]
...
export type AiVideoLoginAttempt = typeof aiVideoLoginAttempts.$inferSelect
