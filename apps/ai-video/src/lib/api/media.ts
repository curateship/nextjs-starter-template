...
const updateMediaFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MediaItem> => {
    ...
    const user = await requireUser()
    const workspace = await requireWorkspace(user.id)
    await getOwnedMedia(user.id, workspace.id, data.mediaId)
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(aiVideoMedia.workspaceId, workspace.id)
        )
      )

    const row = await getOwnedMedia(user.id, workspace.id, data.mediaId)
    return serializeMedia(row)
  })
