  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed z-50 rounded-xl border border-foreground/10 bg-card shadow-lg"
        style={{ left: at.x, top: at.y, width: PANEL_WIDTH }}
      ...
      >
        ...
        <div
          ...
          // However far down the window sits, its body ends above the screen's
          // edge — the Place button must never be pushed somewhere unreachable.
          style={{ maxHeight: Math.max(240, window.innerHeight - at.y - 56) }}
        >
          ...
          <div className="grid gap-2">
            ...
            {plan?.volumeCapped ? (
              <p className="text-xs text-muted-foreground">
                The guard is shrinking some buys — the amounts above show it.
              </p>
            ) : null}
          </div>
          ...
          {refusal ? (
            <p className="border-t border-foreground/10 pt-3 text-xs text-red-600 dark:text-red-400">
              {refusal}
            </p>
          ) : null}
