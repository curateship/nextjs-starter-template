  ...
  return (
    <TooltipProvider>
      <PanelLayoutsMenu
        ...
        onCreate={async (name) => {
          created(name)
          setLayouts([{ id: "layout-1", name, horizontal, vertical }])
        }}
        onApply={async (id) => {
          applied(id)
        }}
...
describe("the saved panel layout menu", () => {
  it("saves, switches and confirms deletion", async () => {
    await act(async () => button("Saved panel layouts").click())
    const input = document.querySelector<HTMLInputElement>("#panel-layout-name")
    ...
    await act(async () => button("Save", false).click())
    expect(created).toHaveBeenCalledWith("Reading")

    await act(async () => button("Reading", false).click())
    expect(applied).toHaveBeenCalledWith("layout-1")

    await act(async () => button("Saved panel layouts").click())
    await act(async () => button("Delete Reading").click())
