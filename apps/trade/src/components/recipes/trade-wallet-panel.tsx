...
import {
  loadRecipeWalletAccounts,
  readRecipeWalletAccounts,
} from "@/lib/recipes/trade-wallet-accounts"
import type { NetworkId, ProtocolId } from "@/lib/protocols/contracts"
import { DEFAULT_BACKTEST_PROTOCOL } from "@/lib/recipes/trade-markets"
import { marketsStepFollowingWallet } from "@/lib/recipes/trade-wallet-markets"
import {
  chosenWallet,
  MAX_POT_USD,
  tradeWalletNode,
...
export default function TradeWalletFields({
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const read = await loadRecipeWalletAccounts(attemptKey > 0)
        if (!alive) return
        setAnswer({
  ...
  const held = readRecipeWalletAccounts()
