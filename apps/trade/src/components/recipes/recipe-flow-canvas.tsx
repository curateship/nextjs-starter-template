/**
 * Deliberate Trade-owned fork of the shell automation canvas. Recipes keep the
 * proven drag, edge, zoom and keyboard behavior without changing shell files.
 */
...
import { recipeNodeName } from "@/lib/recipes/registry"
import {
...
import { RecipeCanvasNode } from "./recipe-canvas-node"
            ...
            return (
              <g key={edge.id}>
                <path
                  ...
                  tabIndex={0}
                  role="button"
                  aria-label={`Connection from ${recipeNodeName(source)} ${edge.sourcePort} to ${recipeNodeName(target)}`}
