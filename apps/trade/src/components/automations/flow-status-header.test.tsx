...
const { default: FlowStatusHeader } =
  await import("@/components/automations/flow-status-header")
...
describe("reading a flow", () => {
  ...
  it("dismisses only its own error after a later read succeeds", async () => {
    vi.useFakeTimers()
    loadFlowTrading
    ...
    expect(dismissErrorToast).toHaveBeenCalledWith("flow-read-error")
  })
})
