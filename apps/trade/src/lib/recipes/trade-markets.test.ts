...
describe("how many coins fit", () => {
  it("buys fewer coins the longer the window gets", () => {
    ...
    expect(coinsAllowedFor("4h", 365)).toBeGreaterThan(
      coinsAllowedFor("4h", 1000)
    )
    expect(coinsAllowedFor("4h", 1000)).toBeGreaterThan(
      coinsAllowedFor("4h", 2000)
    )
  })
  ...
  it("fits the most coins at 4h, and fewer either side of it", () => {
    ...
    expect(coinsAllowedFor("1d", 1000)).toBeLessThan(
      coinsAllowedFor("4h", 1000)
    )
    expect(coinsAllowedFor("1h", 1000)).toBeLessThan(
      coinsAllowedFor("4h", 1000)
    )
  })
...
describe("cutting the list down to what fits", () => {
  ...
  it("cuts the list when the window is stretched under it", () => {
    ...
    expect(
      trimMarketsToFit({ days: 365, marketKeys: picked }, "4h", false)
        .marketKeys
    ).toHaveLength(403)
    expect(
      trimMarketsToFit({ days: 1000, marketKeys: picked }, "4h", false)
        .marketKeys
    ).toHaveLength(coinsAllowedFor("4h", 1000))
  })
