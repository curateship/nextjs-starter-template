...
describe("which refusals are the strategy working", () => {
  ...
  it("counts no money and a refused key as problems", () => {
    expect(flowWaitIsProblem("SMART_LADDER_COST")).toBe(true)
...
describe("what gets stored from an error", () => {
  it("keeps the app's own codes", () => {
    expect(flowWaitCode(new Error("SMART_LADDER_COST"))).toBe(
      "SMART_LADDER_COST"
    )
  ...
  it("throws away anything that is not plainly a code", () => {
    ...
    // is stored forever.
    expect(
      flowWaitCode(new Error("failed signing with 0xabc123def456"))
    ).toBe("FLOW_UNKNOWN")
...
describe("the one line at the top of the chip", () => {
  ...
  it("names the coin when one thing needs a person", () => {
    const words = line([wait("ETH", "SMART_LADDER_COST")])
    expect(words).toBe("ETH — not enough free cash to place the whole ladder.")
  })

  it("counts the coins when the same thing refuses many", () => {
    const words = line([
      wait("ETH", "SMART_LADDER_COST"),
      wait("BTC", "SMART_LADDER_COST"),
      wait("SOL", "SMART_LADDER_NO_BASE"),
    ])
    // The two that share a fault are the story; the one waiting is not.
    expect(words).toBe(
      "2 coins — not enough free cash to place the whole ladder."
    )
  })
  ...
  it("says when it will look again once it has stopped asking", () => {
    const hold = { code: "SMART_LADDER_COST", strikes: 4, until: at + 360_000 }
    const words = line(
      [wait("ETH", "SMART_LADDER_COST"), wait("BTC", "SMART_LADDER_COST")],
      0,
      hold
    )
    expect(words).toBe(
      "2 coins — not enough free cash to place the whole ladder. Trying again in about 6 minutes."
    )
  })
...
describe("the exchange asking us to slow down", () => {
  ...
  it("still stops the flow asking, which waiting for a base does not", () => {
    ...
    expect(flowWaitBacksOff("SMART_LADDER_COST")).toBe(true)
  })
  ...
  it("leads the chip even though it is not a fault", () => {
    const at = 1_700_000_000_000
    const list = [
      describeFlowWait("hyperliquid:testnet:ETH", { code: "EXCHANGE_BUSY", at }),
    ]
