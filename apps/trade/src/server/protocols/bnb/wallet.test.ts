...
import { packBnbCredential, verifyBnbWallet } from "./wallet"
import { evmAddressOf, makeEvmWallet } from "@/server/protocols/evm-chain/wallet"
...
describe("BNB Chain wallets", () => {
  it("derives the known address and normalizes exported keys", () => {
    expect(evmAddressOf(secret)).toBe(address)
  ...
  it.each(["bad", "0x1234", `0x${"0".repeat(64)}`, `0x${"f".repeat(64)}`])(
    ...
    (key) => {
      expect(evmAddressOf(key)).toBeNull()
  ...
  it("makes distinct wallets whose keys prove their addresses", async () => {
    const first = makeEvmWallet()
    const second = makeEvmWallet()
