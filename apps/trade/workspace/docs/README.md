...
- `smart-order-windows.md` — the separate Rung 1 exit % for market buys and
  ...
  ladder summary, the DCA exit choices and draggable exit gap, the base stop,
  and the grid window.
- `rules-before-an-entry.md` — the three rules Tyler sets for himself before a
  ...
  Journal keeps.
- `grid-orders.md` — drawing-alert stop losses with red chart lines and labels, linked-line protection and engine rollout requirements; how a grid recycles, whether it buys the dips or sells the
