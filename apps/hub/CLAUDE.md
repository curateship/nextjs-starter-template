...
- Never run `pnpm run build` while Hub's `next dev` server is running in the same worktree. Both commands write to `.next`, which disrupts Turbopack's cache and hot reload and forces a server restart.
