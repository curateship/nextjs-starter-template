...
- In local development, `localhost:3000` is also treated as the Hub admin host, and `/admin` redirects locally to `/admin/sites`
...
3. Admin sidebar structure lives in `site.settings.admin_sidebar`, and sticky-header section nav resolves from the same child links
...
- Platform admin auth lives at `hub.systemeverything.com/login`, with `/admin/sites` as the current admin entry point.
- Tenant-domain `/admin` is not a tenant dashboard yet; it redirects to the canonical Hub sites list for now.
...
- Standard admin create, settings, and block-editor forms should use the shared dashboard modal primitives in `src/components/admin/layout/dashboard/modals.tsx`.
