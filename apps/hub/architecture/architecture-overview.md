...
- Admin dashboard list pages should reuse the shared helpers in `src/components/admin/layout/list/` for bulk selection, tri-state sort buttons, skeleton rows, pagination footers, and simple confirm/error dialogs. Keep each page's row markup, actions, filters, and data-loading rules local.
