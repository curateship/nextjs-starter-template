...
- Platform admin is canonical on `hub.systemeverything.com`; non-Hub `/admin` URLs temporarily redirect to the Hub sites list until a tenant dashboard exists.
- Local development treats `localhost:3000` as the Hub admin host, so `http://localhost:3000/admin` redirects locally to `/admin/sites`.
