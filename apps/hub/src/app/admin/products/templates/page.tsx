...
export default function ProductTemplatesPage() {
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      setLoading(true)
      setError(null)
      const { data, total: totalCount, error: loadError } = await getProductTemplatesBySite(currentSite.id, { page: currentPage, pageSize })
      if (loadError) {
        setError(loadError)
        setLoading(false)
        return
  ...
  async function handleCreate() {
    ...
    const { data, error: createError } = await createProductTemplate({
    ...
    setFormName("")
    if (data) {
      router.push(`/admin/products/templates/${data.id}`)
    }
  }

  async function handleMassDelete() {
    setMassDeleting(true)
    const { error: deleteError } = await deleteProductTemplates(Array.from(selectedIds))
    if (deleteError) {
  ...
  async function handleSelectAll() {
    ...
    const { ids } = await getProductTemplateIdsAction(currentSite.id)
  ...
  function getBlockCount(template: ProductTemplate) {
  ...
  async function handleSetDefault(templateId: string) {
    const { error: defaultError } = await setDefaultProductTemplate(templateId)
    if (defaultError) {
              ...
              {loading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {normalizedSearchQuery ? "No templates match your search." : "No templates yet. Create one to save reusable product layouts."}
                  </p>
                  ...
                  <div key={template.id} className={`p-6 transition-colors ${selectedIds.has(template.id) ? "bg-accent/50" : ""}`}>
                    <div className="grid grid-cols-5 gap-4 items-center">
                      <div className="col-span-2 flex items-center space-x-4">
                        ...
                        />
                        <Link
                          href={`/admin/products/templates/${template.id}`}
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => router.push(`/admin/products/templates/${template.id}`)}
                          title="Edit Template"
                        >
          ...
          <AdminModalBody className="space-y-6 [&_label+input]:mt-2">
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Sales Page Layout"
                onKeyDown={(event) => {
                  if (event.key === 'Enter') {
