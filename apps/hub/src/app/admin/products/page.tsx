...
export default function ProductsPage() {
    ...
    async function loadProducts() {
      ...
      try {
        ...
        const {
          ...
          total: productsTotal,
          error: productsError
        } = await getSiteProductsWithCategoriesAction(currentSite.id, {
          page: currentPage,
          pageSize
        })
        if (productsError) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const searchText =
      `${product.title} ${product.slug} ${product.meta_description ?? ""} ${categoryText}`.toLowerCase()
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    if (productSort.sortColumn === "modified")
      ...
      <AdminLayout>
        <div className="w-full">
          ...
          <Card>
            ...
            <CardTableHeader className="grid-cols-6">
              <div className="col-span-2 flex items-center space-x-4">
                ...
                />
                <AdminSortButton
                  active={productSort.sortColumn === "title"}
                  direction={productSort.sortDirection}
                  onClick={() => productSort.toggleSort("title")}
                >
                  Product
                </AdminSortButton>
              </div>
              <AdminSortButton
                active={productSort.sortColumn === "category"}
                direction={productSort.sortDirection}
                onClick={() => productSort.toggleSort("category")}
              >
                Category
              </AdminSortButton>
              <AdminSortButton
                active={productSort.sortColumn === "status"}
                direction={productSort.sortDirection}
                onClick={() => productSort.toggleSort("status")}
              >
                Status
              </AdminSortButton>
              <AdminSortButton
                active={productSort.sortColumn === "modified"}
                direction={productSort.sortDirection}
                onClick={() => productSort.toggleSort("modified")}
              >
                Modified
              </AdminSortButton>
              <div>Actions</div>
              ...
              {loading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {products.length === 0
                      ? "No products found"
                      : `No ${filterStatus === "all" && filterPrivacy === "all" ? "" : `${filterStatus === "all" ? "" : filterStatus}${filterStatus !== "all" && filterPrivacy === "private" ? ", " : ""}${filterPrivacy === "private" ? "private " : ""}`}products found`}
                  </p>
                ...
                sortedProducts.map((product) => (
                  <div
                    key={product.id}
                    className={`p-6 transition-colors ${productSelection.selectedIds.has(product.id) ? "bg-accent/50" : ""}`}
                  >
                    <div className="grid grid-cols-6 gap-4 items-center">
                      ...
                      <div>
                        <span className="text-sm text-muted-foreground">{formatRelativeDate(product.updated_at)}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        ...
                        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" asChild>
                          <a
                            href={currentSite ? `${getSiteUrl(currentSite)}/products/${product.slug}` : "#"}
                            target="_blank"
          ...
          <ProductSettingsModal
            ...
            initialCategories={
              settingsProductId
                ? productCategories[settingsProductId] || EMPTY_PRODUCT_CATEGORIES
