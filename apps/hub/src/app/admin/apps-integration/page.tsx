    ...
    return [
      {
        ...
        actionLabel: "Launch",
        note: usingSeoDevDefaults ? "Using local SEO dev defaults." : "Launched from Hub with a one-time code."
      }
    ]
  } catch {
    return [
...
export default async function AppsIntegrationPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
        ...
        <div className="w-full">
          ...
          {params.error === "no-access" ? (
            <Alert variant="destructive" className="mx-4 mb-6">
              <AlertTriangle />
              <AlertDescription>This account does not currently have SEO access.</AlertDescription>
            </Alert>
          ) : null}
                ...
                linkedApps.map((app) => (
                  <div key={app.name} className="p-6">
                    <div className="grid grid-cols-7 gap-4 items-center">
                      ...
                      <div className="truncate text-sm text-muted-foreground">{app.appUrl ?? "Not configured"}</div>
                      <div className="text-sm text-foreground">{app.authOwner}</div>
                      <div className="text-sm text-foreground">{app.billingOwner}</div>
                      <div>
                        <Badge variant={app.status === "Linked" ? "default" : "outline"}>{app.status}</Badge>
                      </div>
                      <div>
