...
export const siteDashboardConfig = pgTable('site_dashboard_config', {
...
export const siteDashboardPages = pgTable('site_dashboard_pages', {
  id: uuid('id').defaultRandom().primaryKey(),
...
export const siteDashboardConfigRelations = relations(siteDashboardConfig, ({ one }) => ({
  site: one(sites, {
    fields: [siteDashboardConfig.siteId],
    references: [sites.id],
  }),
}))

export const siteDashboardPagesRelations = relations(siteDashboardPages, ({ one }) => ({
  site: one(sites, {
    fields: [siteDashboardPages.siteId],
    references: [sites.id],
  }),
}))
