...
interface CreateUserPageModalProps {
  siteId: string
  onSuccess: (page: UserPage) => void
  onCancel: () => void
}

export function CreateUserPageModal({ siteId, onSuccess, onCancel }: CreateUserPageModalProps) {
  const [formData, setFormData] = useState<CreateUserUserPageData>({
    title: '',
    slug: '',
    meta_description: '',
    is_default: false,
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (!response.ok || result.error) {
        setError(result.error || 'Failed to create user page')
        return
      }
      ...
      setError('Failed to save user page as draft')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setError('Failed to publish user page')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="is_default">Set as default page</Label>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            The default page is the first page users see when accessing their dashboard
          </p>
        </div>
      </div>
