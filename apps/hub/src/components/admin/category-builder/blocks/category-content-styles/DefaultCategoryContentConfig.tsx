          ...
          <div className="flex gap-4">
            {(['left', 'center'] as const).map((option) => (
              <div key={option} className="flex items-center gap-2">
                <Checkbox
                  id={`category-alignment-${option}`}
                  checked={alignment === option}
                  onCheckedChange={() => onConfigChange('alignment', option)}
                />
                <Label htmlFor={`category-alignment-${option}`} className="text-sm capitalize cursor-pointer">{option}</Label>
              </div>
            ))}
          </div>
        </CardContent>
          ...
          <div className="flex gap-4">
            {([
              ...
              <div key={option.key} className="flex items-center gap-2">
                <Checkbox
                  id={`category-title-size-${option.key}`}
                  checked={titleSize === option.key}
                  onCheckedChange={() => onConfigChange('titleSize', option.key)}
                />
                <Label htmlFor={`category-title-size-${option.key}`} className="text-sm cursor-pointer">{option.label}</Label>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <Label htmlFor="categoryContentMaxWidth" className="text-sm">Max width</Label>
            <Input
              id="categoryContentMaxWidth"
              type="number"
              min="480"
              max="1280"
