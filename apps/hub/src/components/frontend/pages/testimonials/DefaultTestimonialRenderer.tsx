...
export function DefaultTestimonialRenderer({ items, config }: DefaultTestimonialRendererProps) {
  ...
  const renderCard = (item: TestimonialItem, index: number) => (
    <CarouselItem key={item.id || index} className="basis-auto" useCardGroup>
  ...
  return (
    <div className="lg:container">
      <CardGroup className="grid [mask-image:linear-gradient(90deg,transparent,black_10%,black_90%,transparent)]">
        <Carousel opts={{ loop: true }} plugins={[AutoScroll({ startDelay: 500, speed })]}>
          <CarouselContent useCardGroup>{row1.map((item, i) => renderCard(item, i))}</CarouselContent>
        </Carousel>

        {showSecondRow && row2.length > 0 && (
          <div className="hidden md:block">
            <Carousel opts={{ loop: true }} plugins={[AutoScroll({ startDelay: 500, speed, direction: "backward" })]}>
              <CarouselContent useCardGroup>{row2.map((item, i) => renderCard(item, i))}</CarouselContent>
            </Carousel>
          </div>
        )}
      </CardGroup>
