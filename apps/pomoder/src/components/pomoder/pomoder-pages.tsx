...
export function TasksPage() {
  ...
  return (
    <div className="reference-view tasks-reference-view"><div className="tasks-reference-inner">
      ...
      <section className="today-tasks-card"><header><h3>Today</h3><span>{completed} / {pomodoro.tasks.length} done</span></header><div>{pomodoro.tasks.map((task) => <div className="today-task-row" key={task.id}><button className={task.completed ? "completed" : ""} onClick={() => pomodoro.toggleTask(task.id)}>{task.completed ? <Check aria-hidden="true" /> : null}</button><span className={task.completed ? "completed" : ""}>{task.title}</span><button onClick={() => pomodoro.removeTask(task.id)} aria-label={`Remove ${task.title}`}><X aria-hidden="true" /></button></div>)}</div><form onSubmit={(event) => { event.preventDefault(); pomodoro.addTask(title); setTitle("") }}><Plus aria-hidden="true" /><input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} placeholder="Add a task, press Enter…" aria-label="New task" /></form></section>
