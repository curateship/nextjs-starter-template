...
- Profile pictures are deliberately public, matching the display names they sit beside, but the public route serves an id only while it is somebody's current avatar — a private background or soundscape is never readable through it. Uploads are re-checked against file signatures, capped at 2 MB and rate limited per account; replacing or removing a picture deletes the previous asset and queues its object for deletion, and deleting the asset (including from the admin media tools) clears the account's pointer through a foreign key so the coloured initial returns.
