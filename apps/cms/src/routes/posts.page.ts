import { definePage } from "@/lib/pages/page-descriptor"

/**
 * The Posts page's one declaration. A post's own page at /posts/<address>
 * follows this page's on/off switch rather than having one of its own, the
 * same way a listing follows the directory's.
 *
 * Declaring it here is also what stops an admin writing a page at /posts: the
 * written-page check refuses any address a declared page already answers on.
 *
 * `source: "app"` is left out for the reason `directory.page.ts` gives.
 */
export default definePage({
  path: "/posts",
  name: "Posts",
  summary: "This site's published posts, newest first.",
  layout: "marketing",
})
