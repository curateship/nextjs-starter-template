 ...
 * list the date chips and a From and To day. Every one writes to the address,
 * so a filtered page survives a reload and can be sent to somebody.
 *
 * A category chip keeps the view it is on, so the month and one day follow
 * it too. A date chip keeps the category and the place, and drops the page
 * number, because page 3 of the old list is nowhere in the new one.
 */
export function EventFilters({
  ...
  /** Only the upcoming list takes a date filter. */
  showDates: boolean
}) {
  const navigate = useNavigate()
  const kept = { place: current.place, category: current.category }
  ...
  if (!categories.length && !showDates) return null
          ...
          {categories.map((row) => (
            <li key={row.id}>
              <Link
                ...
                activeOptions={exactSearch}
                aria-current={row.slug === current.category ? "page" : undefined}
