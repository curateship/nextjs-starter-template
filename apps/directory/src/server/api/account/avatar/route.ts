...
export async function POST(request: NextRequest) {
  ...
  if (contentLength > AVATAR_MAX_BODY_BYTES) {
    return NextResponse.json({ error: 'Avatar upload request is too large.' }, { status: 413 })
  }

  const bodyBytes = await readLimitedRequestBody(request, AVATAR_MAX_BODY_BYTES)
  if (!bodyBytes) {
    return NextResponse.json({ error: 'Avatar upload request is too large.' }, { status: 413 })
  }
