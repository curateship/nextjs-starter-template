...
import { clampAdminSidebarWidth, DEFAULT_ADMIN_SIDEBAR_WIDTH } from "@/lib/utils/admin-sidebar-width"
...
function SidebarProvider({
  ...
  sidebarWidth = DEFAULT_ADMIN_SIDEBAR_WIDTH,
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    () => ({
      ...
      toggleSidebar,
      sidebarWidth,
      commitSidebarWidth: (width) => onSidebarWidthCommit?.(clampAdminSidebarWidth(width)),
    }),
    [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, sidebarWidth, onSidebarWidthCommit]
  )
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          style={
            {
              "--sidebar-width": `${clampAdminSidebarWidth(sidebarWidth)}px`,
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { commitSidebarWidth, sidebarWidth, state, toggleSidebar } = useSidebar()
  ...
  return (
    <button
      ...
      onPointerMove={(event) => {
        ...
        drag.width = clampAdminSidebarWidth(drag.startWidth + delta)
      ...
      onKeyDown={(event) => {
        if (state === "collapsed" || (event.key !== "ArrowLeft" && event.key !== "ArrowRight")) return
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 touch-none -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring relative flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-visible rounded-md px-2 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-foreground",
        "before:absolute before:top-0 before:bottom-0 before:left-[-10px] before:w-px before:bg-transparent before:transition-colors",
        "data-[active=true]:before:bg-sidebar-foreground/40",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        className
