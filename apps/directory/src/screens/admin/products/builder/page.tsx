...
export default function ProductBuilderRootPage() {
  ...
  // Bounce to the current site's product builder. Uses replace so this
  ...
  useEffect(() => {
    ...
    if (currentSite) {
      router.replace(`/admin/products/builder/${currentSite.id}`)
    } else if (sites.length > 0) {
      router.replace(`/admin/products/builder/${sites[0].id}`)
    } else {
