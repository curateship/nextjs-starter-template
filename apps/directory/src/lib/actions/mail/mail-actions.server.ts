...
export interface SaveMxrouteInput {
  siteId: string
  server: string
  username: string
  apiKey: string
...
export async function getMailDashboardActionImpl(siteId: string): Promise<{ data: MailDashboardData | null; error: string | null }> {
  try {
...
export async function saveMxrouteIntegrationActionImpl(input: SaveMxrouteInput): Promise<AdminActionResult> {
  try {
    ...
    await createOrUpdateIntegration({ data: { siteId: input.siteId, integrationType: 'mxroute', config: {
      server,
      username,
      api_key: apiKey,
...
export async function createMailboxActionImpl(input: CreateMailboxInput): Promise<AdminActionResult> {
  try {
    const site = await getOwnedSite(input.siteId)
    if (!site) return actionFailure('Site not found')
...
export async function setupMailDomainActionImpl(siteId: string): Promise<AdminActionResult> {
  try {
    const site = await getOwnedSite(siteId)
    if (!site) return actionFailure('Site not found')
...
export async function disableMailboxActionImpl(siteId: string, mailboxId: string): Promise<AdminActionResult> {
  try {
    const site = await getOwnedSite(siteId)
    if (!site) return actionFailure('Site not found')
