...
export async function getSponsorReportLinksActionImpl(siteId: string): Promise<{ data: Record<string, SponsorReportLinkStatus>; error: string | null }> {
  try {
 ...
 */
export async function createSponsorReportLinkActionImpl(sponsorId: string): Promise<{ data: { url: string; link: SponsorReportLinkStatus } | null; error: string | null }> {
  try {
...
export async function revokeSponsorReportLinkActionImpl(sponsorId: string): Promise<{ data: SponsorReportLinkStatus | null; error: string | null }> {
  try {
    const row = await getSponsorWithSiteForUser(sponsorId)
    if (!row) return { data: null, error: 'Sponsor not found or access denied' }
