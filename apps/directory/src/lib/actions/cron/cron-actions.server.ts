...
export async function getCronStatusImpl(siteId: string): Promise<{
...
export async function getCronJobsImpl(): Promise<{ data: CronJob[] | null; error: string | null }> {
  try {
...
export async function toggleCronJobImpl(jobId: string, enabled: boolean): Promise<{ success: boolean; error?: string }> {
  try {
    const user = await getAuthenticatedUser()
