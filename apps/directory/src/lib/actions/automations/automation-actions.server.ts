...
export async function getAutomationsBySiteImpl(
  siteId: string,
  options?: ListOptions
): Promise<{
...
export async function getRecentAutomationRunsForUserImpl(limit = 8): Promise<{
...
export async function getRecentAutomationRunsForSiteImpl(siteId: string, limit = 8): Promise<{
  data: DashboardAutomationRun[]
  error: string | null
}> {
  try {
...
export async function getAutomationEditorDataImpl(automationId: string): Promise<{
...
export async function createAutomationImpl(input: { siteId: string; name: string }): Promise<{
...
export async function saveAutomationImpl(input: {
  automationId: string
  name: string
  graph: unknown
}): Promise<{
...
export async function setAutomationStatusImpl(
  automationId: string,
...
export async function duplicateAutomationImpl(automationId: string): Promise<{ data: AutomationListItem | null; error: string | null }> {
  try {
...
export async function deleteAutomationsImpl(automationIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
...
export async function runAutomationNowImpl(automationId: string): Promise<{ data: AutomationRunItem | null; error: string | null }> {
  try {
    const automation = await requireOwnedAutomation(automationId)
    if (!automation) return { data: null, error: 'Automation not found' }
    const graph = parseAutomationGraph(automation.graph)
