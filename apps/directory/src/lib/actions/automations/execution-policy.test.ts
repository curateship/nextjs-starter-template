...
describe('automation execution policy', () => {
  it('derives success, partial, failed, and no-op run outcomes', () => {
    assert.equal(deriveAutomationRunStatus([{ failed: false, createdPost: true }]), 'success')
    assert.equal(deriveAutomationRunStatus([{ failed: true, createdPost: false }, { failed: false, createdPost: true }]), 'partial')
    assert.equal(deriveAutomationRunStatus([{ failed: true, createdPost: false }]), 'failed')
    assert.equal(deriveAutomationRunStatus([{ failed: false, createdPost: false }]), 'noop')
  })
