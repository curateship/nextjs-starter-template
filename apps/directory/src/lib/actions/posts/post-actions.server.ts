 ...
 */
export async function getSitePostsWithCategoriesActionImpl(
  siteId: string,
...
export async function getSitePostsWithMergedBlocksActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }) {
 ...
 */
export async function updatePostActionImpl(postId: string, updates: UpdatePostData): Promise<{ data: Post | null; error: string | null }> {
  try {
 ...
 * Delete a post
 */
export async function deletePostActionImpl(postId: string): Promise<{ success: boolean; error: string | null }> {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<PostRow>(posts, postId, 'Post')
    if (!access.ok) {
 ...
 */
export async function deletePostsActionImpl(postIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
    if (!postIds.length) {
 ...
 * Duplicate a post
 */
export async function duplicatePostActionImpl(postId: string, newTitle: string): Promise<{ data: Post | null; error: string | null }> {
  try {
    if (!newTitle?.trim()) {
 ...
 */
export async function updatePostBlocksActionImpl(postId: string, blocks: Record<string, any>): Promise<{ success: boolean; error: string | null }> {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<PostRow>(posts, postId, 'Post')
    if (!access.ok) {
