...
export type SiteScopeResult = { siteId: string; error: null } | { siteId: null; error: string }
...
export async function getPaginatedMediaActionImpl(
  page: number = 1,
  pageSize: number = 20,
...
export async function scanUnusedMediaActionImpl(
  site_id?: string
...
export async function updateMediaActionImpl(
  mediaId: string,
...
export async function deleteMediaActionImpl(mediaId: string, site_id?: string): Promise<{ success: boolean; error: string | null }> {
  try {
    const user = await getAuthenticatedUser()
...
export async function deleteMediaItemsActionImpl(
  mediaIds: string[],
  site_id?: string
