 ...
 */
export async function getMultiSiteDashboardData(rangeValue: string): Promise<MultiSiteDashboardData> {
...
export async function getTopReferrers(siteId: string, period: string, limit = 10) {
  ...
  const { from, to } = getDateRange(period)
  const safeLimit = clampLimit(limit)
  const appDomain = normalizeAnalyticsHost(import.meta.env.VITE_APP_DOMAIN || 'localhost')
