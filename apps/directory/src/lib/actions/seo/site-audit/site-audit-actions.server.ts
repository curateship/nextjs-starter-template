...
export async function getSiteAuditDataImpl(siteId: string): Promise<ContentAuditItem[]> {
...
export async function getInternalLinkAnalysisImpl(siteId: string): Promise<InternalLinkAnalysis> {
  const user = await getAuthenticatedUser()
...
export async function saveSiteAuditSettingsImpl(siteId: string, seoSettings: SiteSeoSettings) {
  const user = await getAuthenticatedUser()
...
export async function getSiteForAuditImpl(siteId: string) {
  const user = await getAuthenticatedUser()
  if (!user) return null
