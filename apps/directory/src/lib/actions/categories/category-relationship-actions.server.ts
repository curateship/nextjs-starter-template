...
export async function getContentBreadcrumbPreviewActionImpl(
  contentId: string,
...
export async function getCategoryBreadcrumbPreviewActionImpl(
  categoryId: string
): Promise<{ data: FrontendBreadcrumbItem[] | null; error: string | null }> {
  try {
    const user = await getAuthenticatedUser()
 ...
 */
export async function getContentCategoriesActionImpl(contentId: string, contentType: ContentType) {
  try {
    const user = await getAuthenticatedUser()
    if (!user) {
      return { data: null, error: 'Authentication required' }
 ...
 */
export async function bulkAssignCategoriesToContentActionImpl(
  contentId: string,
