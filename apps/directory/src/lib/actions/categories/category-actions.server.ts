 ...
 */
export async function getCategoriesForSiteActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }) {
  try {
 ...
 */
export async function getCategoriesWithCountsActionImpl(siteId: string, options?: { page?: number; pageSize?: number; parentSlug?: string | null }) {
  try {
    // Auth + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedSite(siteId)
    if (!access.ok) {
 ...
 */
export async function createCategoryActionImpl(
  siteId: string,
 ...
 * Update a category
 */
export async function updateCategoryActionImpl(categoryId: string, data: UpdateCategoryData) {
  try {
 ...
 * Delete a category
 */
export async function deleteCategoryActionImpl(categoryId: string) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<CategoryRow>(categories, categoryId, 'Category')
    if (!access.ok) {
 ...
 */
export async function deleteCategoriesActionImpl(categoryIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
 ...
 */
export async function updateCategoryBlockValuesActionImpl(categoryId: string, contentBlocks: Record<string, any>) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<CategoryRow>(categories, categoryId, 'Category')
    if (!access.ok) {
 ...
 */
export async function getCategoriesWithMergedBlocksActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }) {
  const result = await getCategoriesForSiteActionImpl(siteId, options)
