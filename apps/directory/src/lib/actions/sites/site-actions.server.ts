...
export async function getAllSitesActionImpl(): Promise<{ data: Site[] | null; error: string | null }> {
  try {
...
export async function createSiteActionImpl(siteData: CreateSiteData): Promise<{ data: Site | null; error: string | null }> {
  try {
    const user = await getAuthenticatedUser()
...
export async function cloneSiteActionImpl(
  sourceSiteId: string,
...
export async function updateSiteActionImpl(
  siteId: string,
...
export async function deleteSiteActionImpl(siteId: string): Promise<{ success: boolean; error: string | null }> {
  try {
...
export async function getSiteByIdActionImpl(siteId: string): Promise<{ data: Site | null; error: string | null }> {
  try {
    const user = await getAuthenticatedUser()
    if (!user) return { data: null, error: 'Authentication required' }
...
export async function checkSubdomainAvailabilityActionImpl(subdomain: string): Promise<{ available: boolean; suggestion?: string; error: string | null }> {
  try {
    const existing = await db.query.sites.findFirst({
      where: eq(sites.subdomain, subdomain),
      columns: { id: true },
...
export async function updateSiteNavigationActionImpl(siteId: string, navigationData: Record<string, any>): Promise<{ success: boolean; error: string | null }> {
  return updateSiteChromeField(siteId, 'navigation', navigationData)
}

export async function updateSiteFooterActionImpl(siteId: string, footerData: Record<string, any>): Promise<{ success: boolean; error: string | null }> {
