 ...
 */
export async function getTemplateSitesActionImpl(): Promise<{ data: Site[] | null; error: string | null }> {
  try {
 ...
 */
export async function applyThemeToSiteActionImpl(
  siteId: string,
  templateId: string
 ...
 */
export async function deleteTemplateActionImpl(
  templateId: string
): Promise<{ success: boolean; error: string | null }> {
  try {
    const user = await getAuthenticatedUser()
