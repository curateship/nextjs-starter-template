...
export async function getGuidedFormsBySiteImpl(siteId: string, options?: { page?: number; pageSize?: number }) {
  try {
...
export async function getGuidedFormByIdImpl(formId: string) {
  try {
...
export async function createGuidedFormImpl(input: { siteId: string; name: string }) {
  try {
...
export async function updateGuidedFormImpl(formId: string, updates: Partial<Pick<GuidedForm, 'name' | 'slug' | 'headline' | 'subhead' | 'contact_sync_enabled' | 'admin_notification_enabled' | 'admin_notification_email' | 'draft_steps' | 'draft_outcomes'>>) {
  try {
    const access = await requireOwnedContentRow<typeof guidedForms.$inferSelect>(guidedForms as any, formId, 'Form')
    if (!access.ok) return { data: null, error: access.error }
...
export async function publishGuidedFormImpl(formId: string) {
  try {
    const access = await requireOwnedContentRow<typeof guidedForms.$inferSelect>(guidedForms as any, formId, 'Form')
    if (!access.ok) return { data: null, error: access.error }
...
export async function archiveGuidedFormsImpl(ids: string[]) {
  try {
...
export async function getPublicGuidedFormBySlugImpl(siteId: string, slug: string) {
  try {
...
export async function submitGuidedFormActionImpl(input: {
  formId: string
  versionId: string
  answers: Record<string, any>
  contactEmail?: string
