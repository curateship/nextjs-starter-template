 ...
 */
export async function listUsersImpl(page: number = 1, pageSize: number = 50) {
  try {
 ...
 */
export async function deleteUserImpl(userId: string) {
  try {
    const currentUser = await getAuthenticatedUser()
...
export async function deleteUsersImpl(userIds: string[]) {
  try {
    const currentUser = await getAuthenticatedUser()
