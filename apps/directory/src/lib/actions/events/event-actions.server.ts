...
export async function getSiteEventsActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }) {
  try {
 ...
 */
export async function getSiteEventsWithCategoriesActionImpl(
  siteId: string,
 ...
 */
export async function getSiteEventsWithMergedBlocksActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }) {
  const result = await getSiteEventsActionImpl(siteId, options)
...
export async function updateEventActionImpl(eventId: string, data: UpdateEventData) {
  try {
...
export async function deleteEventActionImpl(eventId: string) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<EventRow>(events, eventId, 'Event')
    if (!access.ok) {
 ...
 */
export async function deleteEventsActionImpl(eventIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
    if (!eventIds.length) {
...
export async function duplicateEventActionImpl(eventId: string, newTitle: string) {
  try {
    if (!newTitle?.trim()) {
 ...
 */
export async function updateEventBlocksActionImpl(eventId: string, contentBlocks: Record<string, any>) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<EventRow>(events, eventId, 'Event')
    if (!access.ok) {
