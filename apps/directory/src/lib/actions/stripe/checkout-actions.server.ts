...
const PAYMENT_INTENT_WINDOW_MS = 15 * 60 * 1000
const PAYMENT_INTENT_MAX_REQUESTS = 20
 ...
 */
export async function createPaymentIntentImpl(data: {
  productId?: string
  productSlug: string
  productName: string
  mainPriceId: string
  ...
  try {
    ...
    const rateLimitKey = `payment-intent:${selection.product.siteId}:${getClientIp(requestHeaders) || 'unknown'}`
    if (await isPersistentRateLimited(rateLimitKey, PAYMENT_INTENT_MAX_REQUESTS, PAYMENT_INTENT_WINDOW_MS)) {
 ...
 */
export async function updatePaymentIntentImpl(data: {
...
export async function updatePaymentIntentCustomerImpl(data: {
  paymentIntentId: string
  email: string
  siteId?: string
}) {
 ...
 */
export async function verifyPaymentIntentImpl(paymentIntentId: string, siteId?: string, productId?: string) {
  try {
