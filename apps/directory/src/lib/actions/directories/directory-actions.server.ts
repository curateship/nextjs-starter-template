...
export interface DirectoryBuilderData {
...
export type DirectoryRow = typeof directories.$inferSelect
...
export async function updateDirectoryActionImpl(directoryId: string, data: UpdateDirectoryInput) {
  try {
...
export async function deleteDirectoryActionImpl(directoryId: string) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<DirectoryRow>(directories, directoryId, 'Directory')
    if (!access.ok) {
 ...
 */
export async function deleteDirectoriesActionImpl(directoryIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
...
export async function duplicateDirectoryActionImpl(directoryId: string, newTitle: string) {
  try {
    if (!newTitle?.trim()) {
...
export async function getDirectoryBuilderDataActionImpl(
  siteId: string,
  ...
  try {
    const [directoryResult, customBlocksResult, optionsResult] = await Promise.all([
      ...
      getDirectoryCustomBlocksBySite({ data: { siteId: siteId } }),
      searchSiteDirectoriesAction({ data: { siteId: siteId, options: {
        limit: 20,
      } } }),
    ])
...
export async function getDirectoryByIdActionImpl(directoryId: string) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<DirectoryRow>(directories, directoryId, 'Directory')
    if (!access.ok) {
...
export async function updateDirectoryBlockValuesActionImpl(directoryId: string, contentBlocks: Record<string, any>) {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<DirectoryRow>(directories, directoryId, 'Directory')
    if (!access.ok) {
