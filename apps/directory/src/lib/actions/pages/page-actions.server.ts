 ...
 */
export async function getSitePagesActionImpl(siteId: string, options?: { page?: number; pageSize?: number; selectedSlug?: string }): Promise<{ data: Page[] | null; total: number; error: string | null }> {
  try {
 ...
 * Delete a page
 */
export async function deletePageActionImpl(pageId: string): Promise<{ success: boolean; error: string | null }> {
  try {
 ...
 */
export async function deletePagesActionImpl(pageIds: string[]): Promise<{ success: boolean; error: string | null }> {
  try {
    if (!pageIds.length) {
 ...
 * Duplicate a page
 */
export async function duplicatePageActionImpl(pageId: string, newTitle: string): Promise<{ data: Page | null; error: string | null }> {
  try {
    if (!newTitle?.trim()) {
 ...
 */
export async function updatePageBlocksActionImpl(pageId: string, contentBlocks: Record<string, any>): Promise<{ success: boolean; error?: string }> {
  try {
    // Auth + row + site ownership (fast-fail helper; check runs on every call)
    const access = await requireOwnedContentRow<PageRow>(pages, pageId, 'Page')
    if (!access.ok) {
