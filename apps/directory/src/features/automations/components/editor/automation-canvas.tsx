...
} from "@/features/automations/domain/catalog";
import { isAutomationConnectionAllowed } from "@/features/automations/domain/graph";
import type {
  AutomationGraph,
...
export function AutomationCanvas({
  ...
  const currentRef = useRef({ graph, connect });
  ...
  function finishConnect(targetId: string) {
    ...
    if (
      !source ||
      !target ||
      !isAutomationConnectionAllowed(source, draft.sourcePort, target)
    ) {
      setConnect(null);
      return;
    }
  ...
  function zoomByFactor(factor: number) {
    ...
    onGraphChange({
      ...
      viewport: {
        zoom,
        x: anchorX - (anchorX - current.x) * (zoom / current.zoom),
        y: anchorY - (anchorY - current.y) * (zoom / current.zoom),
      },
    });
  }
  ...
  function centerViewportAt(x: number, y: number) {
    ...
    onGraphChange({
      ...
      viewport: {
        ...graph.viewport,
        x: canvas.width / 2 - x * graph.viewport.zoom,
        y: canvas.height / 2 - y * graph.viewport.zoom,
      },
    });
  }
  ...
  return (
    <div
      ...
      onDrop={(event) => {
        ...
        const point = worldFromClient(event.clientX, event.clientY);
        onDropNode({
          x: point.x - AUTOMATION_NODE_WIDTH / 2,
          y: point.y - AUTOMATION_NODE_HEIGHT / 2,
        });
      }}
      onPointerDown={(event) => {
      ...
      className={cn(
        "relative h-full min-h-0 touch-none overflow-hidden bg-muted/40 outline-none select-none focus-visible:ring-2 focus-visible:ring-ring/50",
        panning
          ? "cursor-grabbing"
          : connect
            ? "cursor-crosshair"
    ...
    >
      ...
      >
        ...
        >
          {renderedEdges.map(({ edge, source, target }) => {
            const path = edgePath(
              nodeOutputPoint(source, edge.sourcePort),
              nodeInputPoint(target),
            );
            ...
            return (
              <g key={edge.id}>
                <path
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={`Connection from ${source.name} to ${target.name}`}
          ...
          <AutomationCanvasNode
            ...
            onConnectStart={(sourcePort) =>
              setConnect({
                from: node.id,
                sourcePort,
                point: nodeOutputPoint(node, sourcePort),
              })
            }
      ...
      {bounds && minimapScale ? (
        <button
          ...
          className="absolute bottom-3 right-3 hidden overflow-hidden rounded-lg border bg-card/90 shadow-sm backdrop-blur-sm sm:block"
          ...
          {graph.nodes.map((node) => (
            <span
              ...
              style={{
                ...
                width: AUTOMATION_NODE_WIDTH * minimapScale,
                height: AUTOMATION_NODE_HEIGHT * minimapScale,
              }}
            />
          ))}
        </button>
      ...
      <div className="pointer-events-none absolute left-3 top-3 hidden rounded-md border bg-card/90 px-2 py-1 text-[10px] text-muted-foreground backdrop-blur-sm md:block">
        Scroll to zoom · Drag to pan · Drag between ports to connect
      </div>
    </div>
  );
}
