...
const hadOriginalAiAgentsR2PublicUrl = Object.prototype.hasOwnProperty.call(
  process.env,
  "AI_AGENTS_R2_PUBLIC_URL"
)
const originalAiAgentsR2PublicUrl = process.env.AI_AGENTS_R2_PUBLIC_URL

beforeEach(async () => {
  process.env.AI_AGENTS_R2_PUBLIC_URL =
...
afterEach(async () => {
  await client.close()
  if (hadOriginalAiAgentsR2PublicUrl) {
    process.env.AI_AGENTS_R2_PUBLIC_URL = originalAiAgentsR2PublicUrl
  } else {
    delete process.env.AI_AGENTS_R2_PUBLIC_URL
  }
})

describe("ai-agents auth helpers", () => {
  ...
  it("looks up valid sessions and rejects expired or deleted sessions", async () => {
    ...
    await database.insert(aiAgentsSessions).values({
      id: uuid(),
      userId,
    ...
    await expect(findUserBySessionToken(token, database as unknown as AiAgentsDb)).resolves.toMatchObject({
      id: userId,
      email: "tyler@internal.dev",
    })

    await database
      .update(aiAgentsSessions)
      .set({ expiresAt: new Date(Date.now() - 1000) })
    await expect(findUserBySessionToken(token, database as unknown as AiAgentsDb)).resolves.toBeNull()

    await database.delete(aiAgentsSessions)
    await expect(findUserBySessionToken(token, database as unknown as AiAgentsDb)).resolves.toBeNull()
  })
})

describe("ai-agents workspaces", () => {
  it("creates a default workspace and switches the active workspace", async () => {
    ...
    expect(
      await listUserWorkspaces(userId, database as unknown as AiAgentsDb)
    ).toEqual({ workspaces: [], currentWorkspaceId: null })

    const defaultWorkspace = await getOrCreateCurrentWorkspace(
      userId,
      database as unknown as AiAgentsDb
    )
    ...
    await expect(
      listUserWorkspaces(userId, database as unknown as AiAgentsDb)
    ).resolves.toMatchObject({
      currentWorkspaceId: defaultWorkspace.id,
    })

    await expect(
      switchUserWorkspace(userId, uuid(), database as unknown as AiAgentsDb)
...
describe("ai-agents feedback comments", () => {
  it("creates, updates, deletes, counts, and cascades comments", async () => {
    ...
    await database.insert(aiAgentsFeedback).values({
      id: feedbackId,
      userId,
      type: "suggestion",
    ...
    await database.insert(aiAgentsFeedbackComments).values({
      id: commentId,
      feedbackId,
      userId,
    ...
    const remaining = await database.select().from(aiAgentsFeedbackComments)
...
describe("ai-agents feedback notifications", () => {
  it("tracks feedback activity, marks read, and cascades source rows", async () => {
    ...
    await database.insert(aiAgentsFeedbackVotes).values({
      id: voteId,
      feedbackId,
      userId: actorId,
      createdAt,
    ...
    await database.insert(aiAgentsNotifications).values([
      {
    ...
    const [readNotification] = await database
      ...
      .set({ readAt })
      .where(eq(aiAgentsNotifications.id, voteNotificationId))
      .returning()
    expect(readNotification.readAt).toEqual(readAt)

    await database
      .delete(aiAgentsFeedbackVotes)
      .where(eq(aiAgentsFeedbackVotes.id, voteId))
    let remaining = await database.select().from(aiAgentsNotifications)
    expect(remaining.map((row) => row.id)).toEqual([commentNotificationId])

    await database
      .delete(aiAgentsFeedbackComments)
      .where(eq(aiAgentsFeedbackComments.id, commentId))
    remaining = await database.select().from(aiAgentsNotifications)
    expect(remaining).toHaveLength(0)
  })
  ...
  it("paginates only the current user's notifications", async () => {
    ...
    const firstPage = await getNotificationPage({
      ...
      limit: 1,
      database: database as unknown as AiAgentsDb,
    })
  ...
  it("allows only admins to list all notifications", async () => {
    ...
    const [notification] = await database
      ...
      .where(eq(aiAgentsNotifications.id, notificationId))
    expect(notification.readAt).toBeNull()
  })
})

describe("ai-agents media helpers", () => {
  ...
  it("lists only owned media and blocks cross-user access", async () => {
    ...
    await database.insert(aiAgentsMedia).values([
      {
        id: ownedMediaId,
        userId: ownerId,
        filename: "hero.png",
      ...
      media: [
        {
          ...
          url: `https://ai-agents-media.example.test/${ownerId}/hero.png`,
        },
      ],
    })
      ...
      media: [
        {
          ...
          url: `https://ai-agents-media.example.test/${userId}/icon.svg`,
        },
      ],
    })
  })
