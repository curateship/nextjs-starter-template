...
export const SESSION_COOKIE_NAME = "ai_agents_session"
const TEN_YEARS_IN_HOURS = 24 * 365 * 10
const SESSION_TTL_HOURS = Number.parseInt(
  process.env.AI_AGENTS_SESSION_TTL_HOURS || String(TEN_YEARS_IN_HOURS),
  10
)
...
export async function findCurrentUser(database: AiAgentsDb = db) {
...
export async function findUserBySessionToken(
...
): Promise<AiAgentsUser | null> {
  const [session] = await database
    .select()
    .from(aiAgentsSessions)
    .where(
      and(
        eq(aiAgentsSessions.tokenHash, hashSessionToken(token)),
        gt(aiAgentsSessions.expiresAt, now())
      )
    )
    .limit(1)
  ...
  const [user] = await database
    .select()
    .from(aiAgentsUsers)
    .where(eq(aiAgentsUsers.id, session.userId))
    .limit(1)
