...
import { aiAgentsMedia } from "@/server/schema"
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (error instanceof R2StorageNotConfiguredError) {
        throw new Error(
          "R2 storage is not configured. Set the AI_AGENTS_R2_* environment variables, including AI_AGENTS_R2_PUBLIC_URL."
        )
      }
    ...
    try {
      await db.insert(aiAgentsMedia).values(row)
    } catch (error) {
...
const updateMediaFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MediaItem> => {
    ...
    const updatedAt = now()
    await db
      .update(aiAgentsMedia)
      .set({ altText: cleanAltText(data.altText), updatedAt })
      .where(
        and(
          eq(aiAgentsMedia.id, data.mediaId),
          eq(aiAgentsMedia.userId, user.id)
        )
      )
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(aiAgentsMedia.userId, user.id),
          inArray(aiAgentsMedia.id, uniqueIds)
        )
      )
