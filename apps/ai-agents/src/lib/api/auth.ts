...
import { aiAgentsSessions, aiAgentsUsers } from "@/server/schema"
import {
  clearSessionCookie,
...
const loginFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [user] = await db
      .select()
      .from(aiAgentsUsers)
      .where(sql`lower(${aiAgentsUsers.email}) = ${email}`)
      .limit(1)
    ...
    await db.insert(aiAgentsSessions).values({
      id: uuid(),
      userId: user.id,
...
const logoutFn = createServerFn({ method: "POST" }).handler(async () => {
  ...
  if (token) {
    await db
      .delete(aiAgentsSessions)
      .where(eq(aiAgentsSessions.tokenHash, hashSessionToken(token)))
  }
