...
import {
  aiAgentsFeedback,
  aiAgentsFeedbackComments,
  aiAgentsNotifications,
  aiAgentsFeedbackVotes,
  aiAgentsUsers,
  type AiAgentsFeedback,
  type AiAgentsFeedbackComment,
  type AiAgentsUser,
} from "@/server/schema"
...
const listFeedbackFn = createServerFn({ method: "GET" }).handler(
  async (): Promise<FeedbackListResponse> => {
    ...
    const rows = await db
      ...
      .orderBy(desc(aiAgentsFeedback.createdAt))
...
const createFeedbackFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<FeedbackItem> => {
    ...
    await db.insert(aiAgentsFeedback).values(row)
...
const updateFeedbackFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<FeedbackItem> => {
    ...
    const [row] = await db
      .update(aiAgentsFeedback)
      .set({ type: data.type, message, updatedAt: now() })
      .where(eq(aiAgentsFeedback.id, data.feedbackId))
      .returning()
...
const deleteFeedbackFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ feedbackId: string }> => {
    ...
    const [row] = await db
      .delete(aiAgentsFeedback)
      .where(eq(aiAgentsFeedback.id, data.feedbackId))
      .returning({ id: aiAgentsFeedback.id })
...
const deleteFeedbackManyFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ feedbackIds: string[] }> => {
    ...
    const rows = await db
      .delete(aiAgentsFeedback)
      .where(inArray(aiAgentsFeedback.id, data.feedbackIds))
      .returning({ id: aiAgentsFeedback.id })
...
const toggleFeedbackVoteFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<FeedbackItem> => {
    ...
    const [existingVote] = await db
      .select()
      .from(aiAgentsFeedbackVotes)
      .where(
        and(
          eq(aiAgentsFeedbackVotes.feedbackId, data.feedbackId),
          eq(aiAgentsFeedbackVotes.userId, user.id)
        )
      )
      .limit(1)
    ...
    if (existingVote) {
      await db
        .delete(aiAgentsFeedbackVotes)
        .where(eq(aiAgentsFeedbackVotes.id, existingVote.id))
    } else {
      await db.transaction(async (tx) => {
        const createdAt = now()
        const [vote] = await tx
          .insert(aiAgentsFeedbackVotes)
          .values({
            id: uuid(),
          ...
          .returning({ id: aiAgentsFeedbackVotes.id })
        ...
        if (shouldNotifyFeedbackAuthor(row, user)) {
          await tx.insert(aiAgentsNotifications).values({
            id: uuid(),
    ...
    const [voteCount] = await db
      ...
      .from(aiAgentsFeedbackVotes)
      .where(eq(aiAgentsFeedbackVotes.feedbackId, row.id))
    const [author] = await db
      .select({ name: aiAgentsUsers.name })
      .from(aiAgentsUsers)
      .where(eq(aiAgentsUsers.id, row.userId))
      .limit(1)
...
const listFeedbackCommentsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<FeedbackCommentListResponse> => {
    ...
    const rows = await db
      .select()
      .from(aiAgentsFeedbackComments)
      .where(eq(aiAgentsFeedbackComments.feedbackId, data.feedbackId))
      .orderBy(asc(aiAgentsFeedbackComments.createdAt))
...
const listFeedbackCommentDashboardFn = createServerFn({
  ...
  const rows = await db
    .select()
    .from(aiAgentsFeedbackComments)
    .orderBy(desc(aiAgentsFeedbackComments.createdAt))
  ...
  .handler(async ({ data }): Promise<FeedbackCommentItem> => {
    ...
    const row = await db.transaction(async (tx) => {
      const [comment] = await tx
        .insert(aiAgentsFeedbackComments)
        .values({
          id: uuid(),
          feedbackId: data.feedbackId,
          userId: user.id,
...
const updateFeedbackCommentFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<FeedbackCommentItem> => {
    ...
    const [row] = await db
      .update(aiAgentsFeedbackComments)
      .set({ message, updatedAt: now() })
      .where(eq(aiAgentsFeedbackComments.id, data.commentId))
      .returning()
...
const deleteFeedbackCommentFn = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ data }): Promise<{ commentId: string; feedbackId: string }> => {
      ...
      const [row] = await db
        .delete(aiAgentsFeedbackComments)
        .where(eq(aiAgentsFeedbackComments.id, data.commentId))
        .returning({
          id: aiAgentsFeedbackComments.id,
          feedbackId: aiAgentsFeedbackComments.feedbackId,
        })
...
const deleteFeedbackCommentsManyFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ commentIds: string[] }> => {
    ...
    const rows = await db
      .delete(aiAgentsFeedbackComments)
      .where(inArray(aiAgentsFeedbackComments.id, data.commentIds))
      .returning({ id: aiAgentsFeedbackComments.id })
...
async function requireFeedback(feedbackId: string) {
  const [row] = await db
    .select()
    .from(aiAgentsFeedback)
    .where(eq(aiAgentsFeedback.id, feedbackId))
    .limit(1)
...
async function requireFeedbackComment(commentId: string) {
  const [row] = await db
    .select()
    .from(aiAgentsFeedbackComments)
    .where(eq(aiAgentsFeedbackComments.id, commentId))
    .limit(1)
...
export function canManageFeedbackComment(
  comment: Pick<AiAgentsFeedbackComment, "userId">,
  user: Pick<AiAgentsUser, "id" | "role">
) {
  return user.role === "admin" || comment.userId === user.id
}

export function shouldNotifyFeedbackAuthor(
  feedback: Pick<AiAgentsFeedback, "userId">,
  actor: Pick<AiAgentsUser, "id">
) {
  return feedback.userId !== actor.id
}

async function serializeFeedbackRows(
  rows: AiAgentsFeedback[],
  ...
  const voteRows = await db
    .select({
      feedbackId: aiAgentsFeedbackVotes.feedbackId,
      count: sql<number>`count(*)::int`,
    })
    .from(aiAgentsFeedbackVotes)
    .where(inArray(aiAgentsFeedbackVotes.feedbackId, feedbackIds))
    .groupBy(aiAgentsFeedbackVotes.feedbackId)

  const votedRows = await db
    .select({ feedbackId: aiAgentsFeedbackVotes.feedbackId })
    .from(aiAgentsFeedbackVotes)
    .where(
      and(
        inArray(aiAgentsFeedbackVotes.feedbackId, feedbackIds),
        eq(aiAgentsFeedbackVotes.userId, currentUserId)
      )
    )
  ...
  const authorRows = await db
    ...
    .from(aiAgentsUsers)
    .where(inArray(aiAgentsUsers.id, authorIds))
  ...
  const commentRows = await db
    ...
    .from(aiAgentsFeedbackComments)
    .where(inArray(aiAgentsFeedbackComments.feedbackId, feedbackIds))
    .groupBy(aiAgentsFeedbackComments.feedbackId)
...
async function serializeFeedbackWithMeta(
  ...
  const [voteCount] = await db
    ...
    .where(eq(aiAgentsFeedbackVotes.feedbackId, row.id))
  const [vote] = await db
    .select({ id: aiAgentsFeedbackVotes.id })
    .from(aiAgentsFeedbackVotes)
    .where(
      and(
        eq(aiAgentsFeedbackVotes.feedbackId, row.id),
        eq(aiAgentsFeedbackVotes.userId, currentUserId)
      )
    )
    .limit(1)
...
async function serializeFeedbackCommentRows(
  rows: AiAgentsFeedbackComment[],
  currentUser: AiAgentsUser
) {
  if (!rows.length) {
    return []
  }
...
async function getFeedbackCommentCount(feedbackId: string) {
  const [commentCount] = await db
    .select({ count: sql<number>`count(*)::int` })
    .from(aiAgentsFeedbackComments)
    .where(eq(aiAgentsFeedbackComments.feedbackId, feedbackId))
