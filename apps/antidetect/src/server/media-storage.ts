...
function getBucketName() {
  return getR2Setting("ANTIDETECT_R2_BUCKET_NAME")
}

export function getPublicMediaUrl(storagePath: string) {
  const baseUrl = getR2Setting("ANTIDETECT_R2_PUBLIC_URL").replace(/\/+$/, "")
  const key = storagePath.replace(/^\/+/, "")
  return `${baseUrl}/${key}`
}

function getR2Client() {
  const accountId = getR2Setting("ANTIDETECT_R2_ACCOUNT_ID")
  return new S3Client({
    region: "auto",
    endpoint: `https://${accountId}.r2.cloudflarestorage.com`,
    credentials: {
      accessKeyId: getR2Setting("ANTIDETECT_R2_ACCESS_KEY_ID"),
      secretAccessKey: getR2Setting("ANTIDETECT_R2_SECRET_ACCESS_KEY"),
    },
  })
}
